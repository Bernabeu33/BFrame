...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        private void ExecuteBuild()
        {
            ...
            if(m_CopyToStreaming.state)
                DirectoryCopy(m_UserData.m_OutputPath, m_streamingPath+"/AssetBundles/");
            
            MergeEditor.MergeBundleTool.GenerageVersionList();
        }
        ...
        {
#if UNITY_ANDROID
            m_UserData.m_BuildTarget = ValidBuildTarget.Android;
#elif UNITY_IOS
                m_UserData.m_BuildTarget = ValidBuildTarget.iOS;
#endif
            
            m_UserData.m_OutputPath = string.Format("AssetBundles/{0}/AssetBundles", m_UserData.m_BuildTarget.ToString()) ;
            m_UserData.m_Compression = CompressOptions.StandardCompression;
            m_ForceRebuild.state = true;
            m_CopyToStreaming.state = true;
            foreach (var item in m_ToggleData)
            {
                item.state = false;
            }
        }
        
        public void SetAppendHash(bool appendHash)
        {
            foreach (var tog in m_ToggleData)
            {
                if (tog.option != BuildAssetBundleOptions.AppendHashToAssetBundleName) continue;
                tog.state = appendHash;
                break;
            }
        }
        
        public void SetCompressType(CompressOptions compressType)
        {
            m_UserData.m_Compression = compressType;
        }
        ...
        public static void BuildPathMapping()
        {
            string[] allBundleNames = AssetBundleModel.Model.DataSource.GetAllAssetBundleNames();
            List<string> mappingList = new List<string>();
            Regex regex = new Regex("^Atlas/.*/res/");
            foreach (string bundleName in allBundleNames)
            {
                if (bundleName == "lua.assetbundle")
                {
                    continue;
                }
                string[] assetPaths = AssetBundleModel.Model.DataSource.GetAssetPathsFromAssetBundle(bundleName);

                foreach (string assetPath in assetPaths)
                {
                    string packagePath = AssetBundleConfig.AssetsPathToPackagePath(assetPath);
                    //图集资源不需要加
                    if (!regex.IsMatch(packagePath))
                    {
                        string mappingItem = string.Format("{0}{1}{2}", bundleName, AssetBundleConfig.CommonMapPattren, packagePath);
                        mappingList.Add(mappingItem);
                    }
                }
            }
            mappingList.Sort();
            // Assets/AssetsPackage/AssetsMap.bytes
            ...
            {
                Debug.Log("Path Mapping Success!");
            }
        }
    }
